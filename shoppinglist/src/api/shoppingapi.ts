...
apiClient.interceptors.response.use(
  (response) => response,
  (error) => {
    console.log("API Error : ", error.resposne || error.message || error);
    return Promise.reject(
      error.response?.data || new Error(error.message || "예상 못한 에러 발생")
    );
  }
);
...
export const login = async (creds: AccountCredentials): Promise<string> => {
  try {
    ...
    console.log("username / password 가 틀렸습니다.");
    // error 객체가 AxiosError 인스턴스인지 확인하는 과정 작성하겠습니다.
    if (axios.isAxiosError(error) && error.response) {
      throw new Error(
        error.response.data?.message ||
          error.response.data?.error ||
          `Login 실패(${error.response.status})`
      );
...
export const authenticateWithGoogleToken = async (
  ...
  console.log("Google ID 토큰을 백엔드로 전송합니다.", idToken.substring(0, 30) + "..."); // 토큰 전체 노출을 안하기 위해
